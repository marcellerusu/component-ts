...
const Bacon = __webpack_require__(4);
const _ = __webpack_require__(7);
const morphdom = __webpack_require__(8);
var Component;
(function (Component) {
...
(function (Dom) {
    ...
    const event = (type) => (action) => ({ type, action });
    ...
    Dom.A = (attributes, children = [emptyElement]) => ({
        type: 'A',
        assignables: attributes,
        children
    });
    Dom.Button = (attributes, children = [emptyElement]) => ({
        type: 'Button',
        assignables: attributes,
        children
    });
...
const Increment_1 = __webpack_require__(9);
var Root;
(function (Root) {
    const Render = (state) => component_ts_1.Dom.Div([], [
        ...state.incrementers.map(Increment_1.Increment.Render),
        component_ts_1.Dom.Button([component_ts_1.Dom.OnClick({ kind: 'AddIncrementer' })], [component_ts_1.Dom.Text('Add')])
    ]);
    const Update = (state, action) => {
        switch (action.kind) {
            case 'AddIncrementer':
                ...
                return {
                    incrementers: [...state.incrementers, Increment_1.Increment.Create(id)]
                };
            case 'Increment':
            case 'Decrement':
                return {
                    incrementers: state.incrementers.map(incr => incr.id === action.data
                        ? Increment_1.Increment.Update(incr, action)
                        : incr)
                };
            default:
                return state;
...
var Increment;
(function (Increment) {
    Increment.Render = ({ count, id }) => component_ts_1.Dom.Div([], [
    ...
    Increment.Update = (state, { kind, data }) => {
        const id = data;
        if (id !== state.id)
            return state;
        switch (kind) {
    ...
    Increment.Create = (id, startCount = 0) => ({ id, count: startCount });
